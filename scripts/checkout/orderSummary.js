...
export function renderOrderSummary(updatingProductId, updateString) {
  ...
  cart.forEach(cartItem => {
    ...
    const updateClicked = updatingProductId === productId;

    cartSummaryHTML +=
  `
    <div class="cart-item-container js-cart-item-container-${matchingProduct.id}
    js-cart-item-container
    ${updateClicked ? updateString : ''}">
    ...
    .forEach(link => {
      link.addEventListener('click', () => {
        const productId = link.dataset.productId;
          ...
          const updateString = 'is-editing-quantity';
          renderOrderSummary(productId, updateString);
  ...
  document.querySelectorAll('.save-quantity-link')
    .forEach(link => {
      link.addEventListener('click', () => {
        const isSaved = saveQuantity(link);
          if (isSaved) {
            renderOrderSummary();
            renderPaymentSummary();
            renderCheckoutHeader();
  ...
  document.querySelectorAll('.js-quantity-input')
    .forEach(inputElement => {
      ...
      inputElement.addEventListener('keydown', event => {
        ...
        if (event.key === 'Enter') {
          const isSaved = saveQuantity(inputElement);
          if (isSaved) {
            renderOrderSummary();
            renderPaymentSummary();
            renderCheckoutHeader();
